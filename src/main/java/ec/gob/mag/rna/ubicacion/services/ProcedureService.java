...
public class ProcedureService {
	...
	@Transactional
	public List<Localizacion> findPlaceParroquia(Integer ubiId, Double longitud, Double latitud) {
	...
	@SuppressWarnings("unchecked")
	@Transactional
	public List<LocalizacionCanton> findPlaceCanton(Integer ubiId, Double longitud, Double latitud) {
		Session session = entityManager.unwrap(Session.class);
		ProcedureCall call = session.createStoredProcedureCall("sc_geografico.sp_valida_punto_geografico_canton");
		call.registerParameter(1, void.class, ParameterMode.REF_CURSOR);
		call.registerParameter(2, Integer.class, ParameterMode.IN).bindValue(ubiId);
		call.registerParameter(3, Double.class, ParameterMode.IN).bindValue(longitud);
		call.registerParameter(4, Double.class, ParameterMode.IN).bindValue(latitud);
		Output output = call.getOutputs().getCurrent();
		List<LocalizacionCanton> localizaciones = new ArrayList<>();
		if (output.isResultSet()) {
			List<Object[]> resultSet = ((ResultSetOutput) output).getResultList();
			localizaciones = resultSet.stream().map(r -> {
				LocalizacionCanton loc = new LocalizacionCanton((Integer) r[0], (String) r[1], (String) r[2],
						(Integer) r[3], (String) r[4], (String) r[5], (String) r[6], (String) r[7], (Integer) r[8],
						(Double) r[9], (Double) r[10], (Integer) r[11], (String) r[12], (Integer) r[13]);
				return loc;
			}).collect(Collectors.toList());
		}
		return localizaciones;
